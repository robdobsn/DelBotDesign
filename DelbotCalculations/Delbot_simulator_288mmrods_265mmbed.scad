...
hbase = 70; // height of the base structure
htop  = 70;  // height of top structure
htotal = 700; // total height, including base and top structure
bed_level = 11; // distance between the top of the bottom structure and the top of the bed.
extrusion = 8;
rod_space = 1; //if > 0 set two rods instead of one extrusion, diameter based on extrusion
...
//-- Carriage data --------------------------------------------------
car_hor_offset=24; //Carriage: horizontal distance between the articulation and the internal of the columns
car_vert_dist=33;  //Carriage: vertical articulation distance/reference plane (at the top of the carriage) 
hcar=75; // carriage height - no effect on travel
dia_ball = 6; 
dia_arm  = 6;

//-- Effector data --------------------------------------
eff_hor_offset=31; //Distance effector center/articulations Kossel mini: 20, Rostock max: 33mm
eff_vert_dist = 6; //Vertical distance between the bottom of effector and the articulations axis

//-- General design data -------------------------------
arm_space=47; // space between the arms
top_clearance=45; // clearance between top of the carriage and top structure
...
arm_length = 288; // Alternatively, you could define the arm length, which will supersedes the design angle - If too long, you will have problem with the animation (reachable area too large drive to overpass vertical for the arms). Set to 0 give priority to the angle. arm length is given axis to axis. 
...
//Frame details
frame_corner_radius=4.5*extrusion; //modify corner shape radius.
